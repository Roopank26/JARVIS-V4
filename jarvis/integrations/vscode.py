...
class VSCodeBridge:
            ...
            try:
                ...
                if msg_type == MessageType.REQUEST:
                    ...
                    if handler:
                        ...
                        await self._send_response(message["id"], result=result)
                    else:
                        await self._send_response(message["id"], error=f"Unknown method: {method}")
    ...
    async def _send_response(self, msg_id: str, result: Any = None, error: str = None) -> None:
        ...
        try:
            self._client_writer.write((json.dumps(response) + "\n").encode())
    ...
    async def send_notification(self, method: str, params: dict = None) -> None:
        ...
        try:
            self._client_writer.write((json.dumps(message) + "\n").encode())
            await self._client_writer.drain()
        except Exception as e:
    ...
    async def insert_text(self, text: str, position: dict = None) -> bool:
        ...
        try:
            await self.send_notification("jarvis.insertText", {"text": text, "position": position})
            return True
        except Exception as e:
    ...
    async def replace_selection(self, text: str) -> bool:
        ...
        try:
            await self.send_notification("jarvis.replaceSelection", {"text": text})
            return True
        except Exception as e:
    ...
    async def run_command(self, command: str, args: list = None) -> bool:
        ...
        try:
            await self.send_notification(
                "jarvis.executeCommand", {"command": command, "args": args or []}
            )
            return True
        except Exception as e:
    ...
    async def get_terminal_output(self) -> str | None:
    ...
    async def run_in_terminal(self, command: str, cwd: str = None) -> bool:
        ...
        try:
            await self.send_notification("jarvis.runInTerminal", {"command": command, "cwd": cwd})
            return True
        except Exception as e:
