...
def run_platform_diagnostics() -> DiagnosticRunner:
    ...
    runner.add(DiagnosticResult("Platform Detection", True, f"Detected: {platform.value}"))

    # Platform info
    info = get_platform_info()
    runner.add(
        DiagnosticResult("Data Directory", info.data_dir is not None, f"Path: {info.data_dir}")
    )

    runner.add(
        DiagnosticResult("Temp Directory", info.temp_dir is not None, f"Path: {info.temp_dir}")
    )
...
def run_audio_diagnostics() -> DiagnosticRunner:
    ...
    try:
        runner.add(
            DiagnosticResult("Audio Module", True, f"Audio module loaded: {CrossPlatformAudio}")
        )
    except Exception as e:
        runner.add(DiagnosticResult("Audio Module", False, f"Failed to load: {e}"))
        return runner
    ...
    # Audio test
    if len(mics) > 0:
        runner.add(DiagnosticResult("Microphone Access", True, f"Using: {mics[0]['name']}"))
...
def run_tray_diagnostics() -> DiagnosticRunner:
    ...
    runner.add(
        DiagnosticResult(
            "System Tray", available, "Available" if available else "Not available on this platform"
        )
    )
    ...
    try:
        ...
        runner.add(DiagnosticResult("Tray Icon Creation", False, f"Error: {e}"))
...
def run_vscode_diagnostics() -> DiagnosticRunner:
    ...
    runner.add(DiagnosticResult("VS Code Extension Path", True, f"Path: {ext_path}"))

    # Settings path
    settings_path = CrossPlatformVSCode.get_settings_path()
    runner.add(DiagnosticResult("VS Code Settings Path", True, f"Path: {settings_path}"))
...
def run_jarvis_diagnostics() -> DiagnosticRunner:
    ...
    # Test imports
    try:
        runner.add(DiagnosticResult("JARVIS Core", True, "DesktopAssistant loaded"))
    except Exception as e:
        runner.add(DiagnosticResult("JARVIS Core", False, f"Failed: {e}"))
        return runner

    # Test voice components
    try:
        runner.add(DiagnosticResult("Voice Listener", True, "Listener module loaded"))
    except Exception as e:
        runner.add(DiagnosticResult("Voice Listener", False, f"Failed: {e}"))

    # Test memory components
    try:
        runner.add(DiagnosticResult("Long-Term Memory", True, "Memory module loaded"))
    except Exception as e:
        runner.add(DiagnosticResult("Long-Term Memory", False, f"Failed: {e}"))

    # Test scheduler
    try:
        runner.add(DiagnosticResult("Task Scheduler", True, "Scheduler module loaded"))
    except Exception as e:
        runner.add(DiagnosticResult("Task Scheduler", False, f"Failed: {e}"))

    # Test plugins
    try:
        runner.add(DiagnosticResult("Plugin System", True, "Plugin module loaded"))
    except Exception as e:
        runner.add(DiagnosticResult("Plugin System", False, f"Failed: {e}"))
...
def main():
    ...
    parser.add_argument(
        "--category",
        choices=["platform", "audio", "tray", "service", "vscode", "jarvis", "all"],
        default="all",
        help="Category to diagnose",
    )
