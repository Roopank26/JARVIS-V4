...
class ResearchAgent:
    ...
    async def research(
        ...
        # Generate citations
        result.citations = [
            source.to_citation(citation_format) for source in result.sources
        ]
    ...
    def _get_provider_for_url(self, url: str) -> Optional[WebSearchProvider]:
    ...
    async def generate_report(
        ...
        if research.conflicting_info:
            ...
            for conflict in research.conflicting_info:
                lines.append(f"- **{conflict['type']}** between {conflict['source1']} and {conflict['source2']}")
            lines.append("")
