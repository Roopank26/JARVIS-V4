...
class WeatherPlugin(IntentPlugin):
    ...
    async def handle_intent(self, text: str, context: dict[str, Any]) -> str:
        ...
        # Extract location
        location_match = re.search(r"(?:in|for|at)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)", text)
    ...
    async def execute_tool(self, tool_name: str, args: dict[str, Any]) -> Any:
        ...
        if tool_name == "calculate":
            ...
            return self._convert_units(
                args.get("value", 0), args.get("from_unit", ""), args.get("to_unit", "")
            )
...
class NotesMemoryPlugin(MemoryPlugin):
    ...
    async def search(self, query: str) -> list[Any]:
        ...
        for key, note in self._notes.items():
            if query_lower in key.lower() or query_lower in str(note["value"]).lower():
