...
class SelfImprovementLogs:
    ...
    def _load(self) -> None:
        ...
        if self.storage_path.exists():
            try:
                ...
                self.patterns = {
                    k: LearnedPattern.from_dict(v) for k, v in data.get("patterns", {}).items()
                }
    ...
    def _update_pattern(self, input_text: str, intent: str, outcome: Outcome) -> None:
    ...
    def _update_stats(self, log: InteractionLog) -> None:
        ...
        if today not in self._stats["daily"]:
            self._stats["daily"][today] = {"total": 0, "success": 0, "failed": 0, "intents": {}}
        ...
        self._stats["daily"] = {k: v for k, v in self._stats["daily"].items() if k >= cutoff}
    ...
    def get_intent_stats(self) -> dict[str, Any]:
        ...
        for intent in intent_counts:
            ...
            successes = sum(
                1 for i in self.interactions if i.intent == intent and i.outcome == Outcome.SUCCESS
            )
            intent_success[intent] = successes / total if total > 0 else 0

        return {"counts": dict(intent_counts.most_common(20)), "success_rates": intent_success}
    ...
    def get_daily_stats(self, days: int = 7) -> list[dict]:
        ...
        for i in range(days):
            ...
            if date in self._stats.get("daily", {}):
                ...
                stats.append(
                    {"date": date, "total": 0, "success": 0, "failed": 0, "success_rate": 0}
                )
    ...
    def get_patterns(self, min_frequency: int = 2) -> list[dict]:
        ...
        patterns = [p.to_dict() for p in self.patterns.values() if p.frequency >= min_frequency]
    ...
    def get_common_commands(self, limit: int = 10) -> list[dict]:
        ...
        return [{"command": cmd, "count": count} for cmd, count in commands.most_common(limit)]
