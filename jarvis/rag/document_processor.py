...
class DocumentProcessor:
    ...
    def _split_into_chunks(
        ...
        while start < text_length:
            ...
            if (
                start <= (chunk_index - 1) * (self.chunk_size - self.chunk_overlap)
                if chunks
                else start
            ):
    ...
    async def _process_pdf(self, file_path: Path) -> ProcessedDocument:
        ...
        try:
            ...
            for page in doc:
                ...
                if text.strip():
                    page_chunks = self._split_into_chunks(
                        text, str(file_path), {**metadata, "page": page_num}
                    )
            ...
            try:
                ...
                raise ImportError("Install PyMuPDF or PyPDF2 for PDF support") from err
...
class StudyAssistant:
    ...
    async def generate_questions(
        self, file_path: Path, num_questions: int = 5, question_type: str = "general"
    ) -> list[str]:
